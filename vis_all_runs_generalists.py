...
def visualize_runs_for_enemy_group(ax, enemy_group, ea_name, results_dir):
    ...
    for run_num in range(1, 11):  # Iterate through 10 runs
        ...
        result_file = os.path.join(
            results_dir, f"E{group_string}", f"run{run_num}", "results.txt"
        )
...
if __name__ == "__main__":
    ea_simple_dir = "runs_eaSimple_generalist"
    # ea_muPlusLambda_dir = 'runs_muPlusLambda'

    # Enemies to visualize
    enemy_groups = [[1, 2, 3], [1, 4], [4, 5, 6]]

    visualize_ea_for_enemies("eaSimple", ea_simple_dir, enemy_groups)
